...
**Mode:** feature  
Plan ready for review. Approve to begin **Phase 0** (ADR) then **Phase A** (harness).  
Lead agent follows **§18 Execution protocol**.

On approval, reply with:
- Answers to open questions (or "defaults OK")
- Any cast/topology changes
- `skip plan` / `just do it` to proceed immediately with defaults
